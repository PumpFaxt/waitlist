  ...
  switch (functionName) {
    ...
    case "getReferrer":
      return useQuery({
        ...
        staleTime: 1 * DAY,
        queryFn: async () => {
          const resp = await apiClient.get<{ name: string }>(`/user/referrer`);
          return resp.data;
        },
    ...
    case "getPoints":
      return useQuery({
        ...
        staleTime: 10 * MINUTE,
        queryFn: async () => {
          const resp = await apiClient.get<{ points: number }>(`/user/points`);
          return resp.data;
        },
        enabled: !!user?.id,
      });
    ...
    case "getLeaderboard":
      return useQuery({
        ...
        queryFn: async () => {
          const resp = await apiClient.get<{ id: number; points: number }[]>(
